...
module.exports = class ServiceContainer {
  ...
  async stop() {
    for (const [name, mod] of this.defines.entries()) {
      await teardownService(this.app, name, mod);  // eslint-disable-line
    }
  }
};
...
async function startupService(app, name, mod) {
  if (typeof mod.startup === 'function') {
    debug('startup service: %s', name);
    await mod.startup(app);
  }
}

async function teardownService(app, name, mod) {
  if (typeof mod.teardown === 'function') {
    debug('teardown service: %s', name);
    await mod.teardown(app);
  }
}
